...
user_name = input('Enter user account name: ') #lukmanae99
tweets = collection.find({"user.screen_name": user_name})
sanitized = json.loads(json_util.dumps(tweets))
normalized = pd.json_normalize(sanitized)
df_tweets = pd.DataFrame(normalized)

df = pd.read_excel('data_uji.xlsx', "Sheet6")
...
# personality['index'] = personality.index
